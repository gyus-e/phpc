...
__global__ void matmat_tiled(const double *__restrict__ A,
                             const double *__restrict__ B,
                             double *__restrict__ C, const size_t N,
                             const size_t M, const size_t K, const size_t ldA,
  ...
  for (int i = 0; i < K; i += BLOCK_SIZE) {
    ...
    res += dotProduct(shared_A, shared_B, BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE,
                      localRow, localCol);
    __syncthreads();
  }
